...
for c in cnts:
    ...
    if number == 1:
        cy1 = cY
        cv2.line(imgA, (0, cY-30), (heightImgResize, cY-30), (255, 255, 255), 1)
        cv2.line(imgA, (0, cY), (heightImgResize, cY), (255, 255, 255), 1)
        cv2.line(imgA, (0, cY + 25), (heightImgResize, cY + 25), (255, 255, 255), 1)
    ...
    if(cY >= cy1-35 and cY <= cy1+25):
        if(h > heightMaxHarokat):
            crop_imgHuruf = imgA[0:heightImgResize, x:x + w]
            cv2.imshow("cropped{}".format(number), crop_imgHuruf)
            # yStart = y
            # yEnd = y + h
            # xStart = x
            # xEnd = x + w
            # image, cnts, hierarchy = cv2.findContours(crop_imgHuruf, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            # (cnts, _) = contours.sort_contours(cnts)
            # numberA=0
            # for c1 in cnts:
            #
            ...
            #
            #     if(cYa <= cy1-35 or cYa >= cy1+25):
            #         if(xa >= xStart and xa+wa <= xEnd):
            #             numberA += 1
            #             cv2.line(crop_imgHuruf, (xa, 0), (xa, heightImgResize), (255, 255, 255), 2)
            #             cv2.line(crop_imgHuruf, (xa + wa, 0), (xa + wa, heightImgResize), (255, 255, 255), 2)
            #             # if(ya>yStart):
            #             #     yStart = ya
            #             # if(ya + ha < yEnd):
            #             #     yEnd = ya + ha
            ...
            #             cv2.imshow("cropped{}".format(numberA), crop_imgHuruf2)
            #             xStart = xEnd
